...
def truncate_at_sentence(text: str, max_chars: int) -> str:
    ...
    Returns:
        Truncated text ending at sentence boundary with ellipsis, or original text if shorter
    """
    ...
    # Look for sentence endings (. ! ?) followed by space, working backwards
    # Reverse search for sentence endings
    match = None
    for i in range(len(truncated) - 1, max(0, int(max_chars * 0.7)), -1):
        if truncated[i] in '.!?' and i + 1 < len(truncated) and truncated[i + 1] == ' ':
            match = i + 1
            break

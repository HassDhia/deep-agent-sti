...
# Route-aware composition functions for router integration
...
def compose_market_report(
    ...
    """
    Compose market report using EnhancedSTIAgent.
    ...
    Returns:
        Report dict with sections, confidence, metadata
    """
    try:
        # Call agent's search method
        markdown_report, json_ld_artifact, run_summary = agent.search(
            query=query,
            days_back=days,
            seed=42,  # Use default seed
            budget_advanced=0
        )
        ...
        # Extract executive summary (first section or from run_summary)
        exec_summary = ""
        if sections:
            exec_summary = sections[0].get("md", "")[:500]  # First 500 chars
        
        # Get confidence from run_summary or json_ld
        ...
        logger.error(f"Error composing market report: {e}")
        # Return fallback
        return {
            "query": query,
            "path": "market",
            "exec_summary_md": f"Market analysis for '{query}' (error: {str(e)})",
            "sections": [],
            "confidence": {"score": 0.0, "breakdown": {}},
            "sources": [],
            "metadata": rationale
        }


def compose_thesis_report(
    query: str, 
    rationale: dict, 
    agent,
    shallow: bool = False
    ...
    """
    Compose thesis report using EnhancedSTIAgent.
    ...
    try:
        ...
        markdown_report, json_ld_artifact, run_summary = agent.search(
            ...
            days_back=days_back,
            seed=42,
            budget_advanced=10000 if not shallow else 0  # Auto-allocate for thesis
        )
        ...
        exec_summary = ""
        if sections:
            exec_summary = sections[0].get("md", "")[:500]
        ...
        logger.error(f"Error composing thesis report: {e}")
        # Return fallback
        return {
            "query": query,
            "path": "thesis",
            "exec_summary_md": f"Thesis analysis for '{query}' (error: {str(e)})",
            "sections": [],
            "confidence": {"score": 0.0, "breakdown": {}},
            "sources": [],
            "metadata": rationale
...
def _parse_markdown_sections(markdown: str) -> list:
    ...
    for line in lines:
        ...
        if line.startswith("##"):
            ...
            if current_section:
                sections.append({
                    "id": current_section.lower().replace(" ", "-"),
    ...
    # If no sections found, create one from entire markdown
    if not sections:
        sections.append({
            "id": "content",
            "title": "Report",
...
def _extract_sources_from_jsonld(json_ld: dict) -> list:
    """Extract sources from JSON-LD artifact"""
    sources = []
    
    if "hasPart" in json_ld:
        for i, part in enumerate(json_ld["hasPart"], 1):
            sources.append({
                "id": i,
                "title": part.get("headline", ""),
                "url": part.get("url", ""),
                "publisher": part.get("publisher", {}).get("name", ""),
                "date": part.get("datePublished", ""),
                "credibility": part.get("confidence", 0.0)
            })
    
    return sources


def build_exec_summary(claims: list) -> str:
    """
    Build executive summary from claims (fallback when LLM fails).
    ...
    """
    if not claims:
        return "No claims available for executive summary."
    
    # Extract claim texts
    claim_texts = []
    for claim in claims[:5]:  # Top 5 claims
        if isinstance(claim, dict):
            claim_texts.append(claim.get("text", "") or claim.get("claim", ""))
        else:

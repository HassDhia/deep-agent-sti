...
try:
    ...
    logger.warning("dateutil not available - install with: pip install python-dateutil")
...
class SimpleMCPTimeFilteredAgent:
    ...
    def __init__(self, openai_api_key: str, tavily_api_key: str = "",
        ...
        # SearXNG health check caching (to avoid repeated checks and log spam)
        self._searxng_health_check_status = None  # None = not checked, True = passed, False = failed
        self._searxng_health_check_error = None
        self._searxng_health_check_time = None  # Track when we last checked
    ...
    def _searxng_search(self, query: str, domains: List[str], source_type: SourceType, max_results: int, days_back: int) -> List[Source]:
        ...
        # Use cached result if available (check once per agent instance, or re-check after 5 minutes)
        import time
        current_time = time.time()
        health_check_cache_valid = (
            self._searxng_health_check_status is not None and
            self._searxng_health_check_time is not None and
            (current_time - self._searxng_health_check_time) < 300  # Cache for 5 minutes
        )
        
        if health_check_cache_valid:
            health_check_passed = self._searxng_health_check_status
            if not health_check_passed:
                # Only log at debug level for subsequent checks (reduce log spam)
                logger.debug(f"SearXNG health check failed (using cached result from {int(current_time - self._searxng_health_check_time)}s ago)")
        else:
            ...
            try:
                ...
                logger.debug("SearXNG health check passed (GET)")
            except Exception as e:
                health_check_error = str(e)
                logger.warning(f"SearXNG health check failed (GET): {e}")
                # try POST w/o time_range
                try:
                    _ = run('test', use_time=False, method='POST')
                    health_check_passed = True
                    logger.debug("SearXNG health check passed (POST)")
                except Exception as e2:
                    health_check_error = f"GET: {health_check_error}, POST: {str(e2)}"
                    # Only log detailed error on first failure (reduce log spam)
                    logger.error(f"SearXNG health check failed for both GET and POST")
                    logger.error(f"  Base URL: {base}")
                    logger.error(f"  Search URL: {url}")
                    logger.error(f"  Category: {category}")
                    logger.error(f"  Error details: {health_check_error}")
                    logger.error(f"  This may indicate SearXNG is not running or unreachable")
                    logger.error(f"  Check: docker ps (if using Docker) or verify SearXNG is running on {base}")
                    logger.error(f"  Quick test: curl '{base}/search?q=test&format=json'")
                    logger.error(f"  This error will be cached for 5 minutes to reduce log spam")
                    logger.warning("⚠ Continuing despite health check failure - searches may fail")
            
            # Cache the result
            self._searxng_health_check_status = health_check_passed
            self._searxng_health_check_error = health_check_error
            self._searxng_health_check_time = current_time
        
        # If health check failed, log a warning but don't abort (only on first check)
        if not health_check_passed:
            if not health_check_cache_valid:  # Only log on first check
                logger.warning("⚠ SearXNG health check failed, but continuing with searches...")
                logger.warning("  If searches fail, check SearXNG connectivity and configuration")
            ...
            try:
                ...
                for r in results:
                    ...
                    try:
                        ...
                        pass
                    # Use improved _extract_date method (checks multiple field names and extracts from content)
                    parsed_date = self._extract_date(r)
                    
                    # If _extract_date returns None, try flexible parsing for academic sources
                    if not parsed_date:
                        # Check if this might be a canonical paper (heuristic: contains common canonical indicators)
                        is_canonical_heuristic = any(indicator in t.lower() for indicator in [
                            'theory', 'foundation', 'seminal', 'classic', 'survey', 'review',
                            'shannon', 'turing', 'von neumann', 'nash', 'kahneman', 'tversky',
                            'granovetter', 'watts', 'strogatz', 'cialdini', 'herman', 'chomsky'
                        ])
                        
                        # Try flexible date parsing as fallback (for academic sources)
                        date_value = r.get('pubdate') or r.get('published') or r.get('date')
                        parsed_date = self._parse_date_flexible(
                            date_value, 
                            source_type, 
                            title=t,
                            is_canonical=is_canonical_heuristic
                        )
                    
                    if not parsed_date:
                        # Only reject if it's not an academic source
                        if source_type != SourceType.ACADEMIC:
                            logger.warning(f"✗ Rejecting source '{t}' - no publication date found")
                            continue
                        else:
                            # This shouldn't happen with flexible parsing, but add safety
                            logger.warning(f"⚠ Academic source '{t}' has no date, but proceeding anyway")
                            parsed_date = getattr(STIConfig, 'ACADEMIC_FALLBACK_DATE', '2000-01-01')
    ...
    def _parse_date_flexible(self, date_value: Any, source_type: SourceType, 
                             title: str = "", is_canonical: bool = False) -> Optional[str]:
        """
        Flexible date parsing that handles academic sources gracefully.
        
        Args:
            date_value: Raw date value from SearXNG (can be str, int, None, etc.)
            source_type: Type of source (ACADEMIC vs others)
            title: Source title for logging
            is_canonical: Whether this is a canonical/foundational paper
            
        Returns:
            Parsed date in YYYY-MM-DD format, or None if parsing fails
        """
        if not date_value:
            # For academic sources, especially canonical papers, allow missing dates
            if source_type == SourceType.ACADEMIC:
                if is_canonical:
                    # Canonical papers may be old and not have dates - use a fallback old date
                    fallback_date = getattr(STIConfig, 'CANONICAL_FALLBACK_DATE', '1900-01-01')
                    logger.debug(f"Canonical paper '{title}' has no date, using fallback: {fallback_date}")
                    return fallback_date
                else:
                    # For other academic sources, try to infer from context or use recent fallback
                    # Use a date far in the past to indicate it's foundational
                    fallback_date = getattr(STIConfig, 'ACADEMIC_FALLBACK_DATE', '2000-01-01')
                    logger.debug(f"Academic source '{title}' has no date, using fallback: {fallback_date}")
                    return fallback_date
            # For non-academic sources, missing date is still a problem
            return None
        
        # Convert to string and clean
        date_str = str(date_value).strip()
        if not date_str or date_str.lower() in ['none', 'null', '']:
            return self._parse_date_flexible(None, source_type, title, is_canonical)
        
        # Try multiple date formats (expanded list for academic sources)
        date_formats = [
            '%Y-%m-%d',                    # Standard ISO format
            '%Y-%m-%d %H:%M:%S',          # Space-separated datetime (e.g., '2025-04-08 00:00:00')
            ...
            '%Y',                           # Just year (for very old papers)
        ]
        
        # Try parsing with each format
        for fmt in date_formats:
            try:
                # Handle year-only format specially
                if fmt == '%Y':
                ...
                # For formats with time, try to parse just the date part first
                if 'T' in date_str and fmt in ['%Y-%m-%dT%H:%M:%S', '%Y-%m-%dT%H:%M:%SZ', 
                ...
                # Try full format parsing
                parsed_date = datetime.strptime(date_str, fmt)
                return parsed_date.strftime('%Y-%m-%d')
            except (ValueError, TypeError):
                continue
        
        # If all formats fail, handle based on source type
        if source_type == SourceType.ACADEMIC:
            # For academic sources, try to extract year from string
            import re
            ...
            if year_match:
                ...
                if 1900 <= year <= datetime.now().year:
                    ...
                    logger.debug(f"Extracted year {year} from '{date_str}' for '{title}', using: {fallback_date}")
                    return fallback_date
            
            # Last resort: use fallback date
            fallback_date = getattr(STIConfig, 'ACADEMIC_FALLBACK_DATE', '2000-01-01') if not is_canonical else getattr(STIConfig, 'CANONICAL_FALLBACK_DATE', '1900-01-01')
            logger.warning(f"Could not parse date '{date_str}' for academic source '{title}', using fallback: {fallback_date}")
            return fallback_date
        
        # For non-academic sources, return None (strict mode)
        logger.warning(f"Could not parse date '{date_str}' for source '{title}'")
        return None
    
    def _is_within_time_window(self, source: Source, start_date: datetime, end_date: datetime) -> bool:
        """
        Check if source is within time window.
        Lenient for academic sources - allows out-of-window academic sources for foundational papers.
        """
        ...
        try:
            ...
            if is_within:
                ...
                logger.debug(f"✓ Source '{source.title}' date {source.date} is within window")
            else:
                self.date_filter_stats['outside_window'] += 1
                days_diff = (datetime.now() - source_date).days
                
                # For academic sources, be more lenient - allow out-of-window sources
                # They may be foundational/canonical papers that are still relevant
                if source.source_type == SourceType.ACADEMIC:
                    # Check if it's a very old date (fallback date indicating unknown/old)
                    canonical_fallback = getattr(STIConfig, 'CANONICAL_FALLBACK_DATE', '1900-01-01')
                    academic_fallback = getattr(STIConfig, 'ACADEMIC_FALLBACK_DATE', '2000-01-01')
                    if source.date in [canonical_fallback, academic_fallback]:
                        logger.info(f"✓ Academic source '{source.title}' has fallback date, allowing (foundational paper)")
                        return True
                    
                    # Allow academic sources that are reasonably recent (within configured max age)
                    max_age_days = getattr(STIConfig, 'ACADEMIC_MAX_AGE_DAYS', 3650)  # Default 10 years
                    if days_diff <= max_age_days:
                        logger.info(f"✓ Academic source '{source.title}' is out of window but recent ({days_diff} days), allowing")
                        return True
                    
                    logger.warning(f"⚠ Academic source '{source.title}' date {source.date} is outside window ({days_diff} days ago) but allowing for foundational value")
                    return True  # Allow academic sources even if out of window
                
                logger.warning(f"✗ Source '{source.title}' date {source.date} is outside window (published {days_diff} days ago)")
                return False
            
        except Exception as e:
            # For academic sources, be lenient with parsing errors
            if source.source_type == SourceType.ACADEMIC:
                self.date_filter_stats['parse_failed'] += 1
                logger.warning(f"⚠ Academic source '{source.title}' has unparseable date '{source.date}', but allowing: {str(e)}")
                return True  # Allow academic sources with parsing errors
            
            # STRICT MODE: For non-academic sources, reject if date parsing fails
            self.date_filter_stats['parse_failed'] += 1
...
"""
    ...
    def _extract_date(self, result: Dict[str, Any]) -> Optional[str]:
        ...
        Based on SearXNG best practices:
        - Checks multiple field name variations (publishedDate, published_date, pubdate, etc.)
        - Uses dateutil.parser for flexible parsing
        - Extracts dates from content/snippet when metadata is missing
        - Handles relative dates like "X days ago"
        """
        # First, try explicit date fields (expanded list based on SearXNG variations)
        date_fields = [
            'publishedDate',      # camelCase (SearXNG)
            'published_date',      # snake_case
            'pubdate',             # common abbreviation
            'published',            # simple field name
            'date',                 # generic date field
            'created_at',           # creation timestamp
            'updated_at',           # update timestamp
            'publishedTime',        # time variant
            'pubDate',              # mixed case
            'publish_date',         # alternative spelling
            'publication_date',     # full form
        ]
        ...
        # If no explicit date fields, try to extract from content/snippet
        content_fields = ['content', 'snippet', 'raw_content', 'description']
    ...
    def _parse_date_string(self, date_str: str) -> Optional[str]:
        ...
        # Handle relative dates like "5 days ago", "2 weeks ago", etc.
        ...
        # If standard formats fail, try dateutil.parser (more flexible)
        if DATEUTIL_AVAILABLE:
            try:
                parsed_date = dateutil_parser.parse(date_str, fuzzy=True, default=datetime.now())
                return parsed_date.strftime('%Y-%m-%d')
    ...
    def _parse_relative_date(self, date_str: str) -> Optional[str]:
        """Parse relative dates like '5 days ago', '2 weeks ago', etc."""
        date_str_lower = date_str.lower().strip()
        
        # Pattern: "X days ago", "X day ago", "X weeks ago", "X week ago"
        patterns = [
    ...
    def _extract_date_from_text(self, text: str) -> Optional[str]:
        ...
        date_patterns = [
            ...
            # US format: 10/13/2025 or 10/13/25
            (r'\b(\d{1,2}/\d{1,2}/\d{4})\b', '%m/%d/%Y'),
            (r'\b(\d{1,2}/\d{1,2}/\d{2})\b', '%m/%d/%y'),
            # European format: 13/10/2025
            (r'\b(\d{1,2}/\d{1,2}/\d{4})\b', '%d/%m/%Y'),
            # Full date: October 13, 2025 or Oct 13, 2025
            (r'\b([A-Za-z]+ \d{1,2}, \d{4})\b', '%B %d, %Y'),
            (r'\b([A-Za-z]{3} \d{1,2}, \d{4})\b', '%b %d, %Y'),
            # Day first: 13 October 2025
            (r'\b(\d{1,2} [A-Za-z]+ \d{4})\b', '%d %B %Y'),
            # Relative dates
            (r'(\d+)\s+days?\s+ago', None),  # Will be handled by _parse_relative_date
        ...
        for pattern, fmt in date_patterns:
            if fmt is None:
                ...
                if match:
                    relative_date = self._parse_relative_date(match.group(0))
                    if relative_date:
                        return relative_date
            else:

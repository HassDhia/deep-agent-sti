"""
Operator Analysis MCP Server

Provides FastMCP tools that transform curated sources and signals into the
sections required by the Brand Collab Lab operator workflow.
"""
...
analysis_mcp = FastMCP("OperatorAnalysis")

organization = os.getenv("OPENAI_ORGANIZATION") or STIConfig.OPENAI_ORGANIZATION
llm = ChatOpenAI(
    api_key=os.getenv("OPENAI_API_KEY"),
    model=STIConfig.DEFAULT_MODEL,
    temperature=STIConfig.MODEL_TEMPERATURE,
    response_format=STIConfig.RESPONSE_FORMAT,
    openai_organization=organization,
)

logger = logging.getLogger(__name__)

REPORT_CONTRACT = STIConfig.REPORT_CONTRACT
SECTION_HEADERS = REPORT_CONTRACT["sections"]
OPERATOR_MOVE_TONE = (
    "Operator move: 1 sentence, cites owner + window + KPI, testable in ≤8 weeks."
)
PARAGRAPH_GUARD = (
    "Paragraphs must be 2-4 sentences long. Break after 4 sentences max."
)
BOLD_USAGE = "Use **bold** only for key numbers or decisive instructions."
MICRO_HEADING_RULE = "Insert an H3 subheading every 3–5 paragraphs for scanners."
STYLE_META_PROMPT = (STIConfig.MARKET_REPORT_META_PROMPT or "").strip()


def _styled_prompt(body: str) -> str:
    prefix = (
        "STI MARKET REPORT META PROMPT (apply to every textual output field):\n"
        f"{STYLE_META_PROMPT}\n\n" if STYLE_META_PROMPT else ""
    )
    return f"{prefix}{body}"


def _loads(payload: str, default: Any) -> Any:
    if not payload:
        return default
    try:
...
def _sources_digest(sources: List[Dict[str, Any]], clip: int = 4200) -> str:
    rows = []
    for source in sources[:12]:
        snippet = source.get("snippet") or source.get("content", "")
        snippet = (snippet or "")[:260].replace("\n", " ")
        rows.append(
            f"[{source.get('id')}] {source.get('publisher')} — {source.get('date')}: "
            f"{source.get('title')}. {snippet}"
        )
    digest = "\n".join(rows)
    return digest[:clip]


def _signals_digest(signals: List[Dict[str, Any]]) -> str:
    rows = []
    for signal in signals[:8]:
        citations = ", ".join(str(cid) for cid in signal.get("citations", []))
        rows.append(
            f"{signal.get('id', 'S?')} | {signal.get('category', 'Signal')}: "
            f"{signal.get('text')} (operator move: {signal.get('operator_move', 'n/a')} "
            f"— cites [{citations}])"
        )
    return "\n".join(rows)


def _quant_digest(quant: Dict[str, Any]) -> str:
    anchors = quant.get("anchors", []) if isinstance(quant, dict) else []
    if not anchors:
        return "No hard metrics surfaced"
    rows = []
    for anchor in anchors[:4]:
        rows.append(
            f"{anchor.get('label')}: {anchor.get('value')} (sources {anchor.get('source_ids', [])})"
        )
    return "\n".join(rows)
...
@analysis_mcp.tool()
def generate_signal_map(sources_json: str, scope_json: str = "") -> str:
    """Create operator-ready signal map clustering signals by behavioral family."""

    sources = _loads(sources_json, [])
    scope = _loads(scope_json, {})
    logger.debug("generate_signal_map invoked (%d sources)", len(sources))
    source_digest = _sources_digest(sources)

    prompt = _styled_prompt(
        f"""
ROLE
...
def generate_deep_analysis(
    sources_json: str, signals_json: str, quant_json: str = "", scope_json: str = ""
) -> str:
    """Produce investigative deep dives (3-5 subsections) with tight sourcing."""

    sources = _loads(sources_json, [])
    signals = _loads(signals_json, [])
    quant = _loads(quant_json, {})
    scope = _loads(scope_json, {})
    logger.debug("generate_deep_analysis (%d sources, %d signals)", len(sources), len(signals))
    prompt = _styled_prompt(
        f"""
ROLE
...
@analysis_mcp.tool()
def generate_pattern_matches(
    sources_json: str, signals_json: str, quant_json: str = "", scope_json: str = ""
) -> str:
    """Map historical and contemporary pattern matches for confidence building."""

    sources = _loads(sources_json, [])
    signals = _loads(signals_json, [])
    quant = _loads(quant_json, {})
    scope = _loads(scope_json, {})
    logger.debug("generate_pattern_matches (%d sources, %d signals)", len(sources), len(signals))
    prompt = _styled_prompt(
        f"""
ROLE
...
def generate_brand_outcomes(
    ...
    quant = _loads(quant_json, {})
    scope = _loads(scope_json, {})
    logger.debug("generate_brand_outcomes (%d sources, %d signals)", len(sources), len(signals))
    prompt = _styled_prompt(
        f"""
ROLE
Translate signals into concrete brand/operator outcomes.
...
@analysis_mcp.tool()
def generate_quant_blocks(
    sources_json: str, signals_json: str, scope_json: str = ""
) -> str:
    """Quantifier agent that surfaces numeric anchors and measurement plans."""
...
@analysis_mcp.tool()
def generate_operator_specs(
    signals_json: str,
    quant_json: str = "",
    scope_json: str = "",
    sections_json: str = "",
) -> str:
    """Build a shared pilot + metric spec for downstream renderers."""

    signals = _loads(signals_json, [])
    quant = _loads(quant_json, {})
    scope = _loads(scope_json, {})
    sections = _loads(sections_json, {})
    operator_job_story = scope.get("operator_job_story") or "Not provided"
    measurement_plan = quant.get("measurement_plan", [])
...
- When the data only includes a single bound (for example, "≥25%"), still populate target_range with two numbers by repeating the bound (for example, [25, 25]) and encode the direction in target_text.
...
@analysis_mcp.tool()
def generate_activation_kit(signals_json: str, sections_json: str, quant_json: str = "") -> str:
    """Build the Activation Kit handoff for Brand Collab Lab."""

    signals = _loads(signals_json, [])
    sections = _loads(sections_json, {})
    quant = _loads(quant_json, {})
    logger.debug(
        "generate_activation_kit (%d signals, sections: %s)",
        len(signals),
        list(sections.keys()),
    )
    prompt = _styled_prompt(
        ...
        "activation_kit": [
            {{
                "display": {{
                    ...
                    "placement_options": ["Example placement"]
                }},
                "ops": {{
                    "operator_owner": "",
                    "collaborator": "",
                    ...
                    "target_map": [{{"org_type": "Retailer", "role": "Retail ops", "why_now": "Example reason"}}],
                    "cadence": [
...
def generate_comparison_map(
    signals_json: str, sections_json: str, scope_json: str = ""
) -> str:
    """Create the comparison/decision map for operators."""

    signals = _loads(signals_json, [])
    sections = _loads(sections_json, {})
    scope = _loads(scope_json, {})
    pattern_matches = sections.get("pattern_matches") or []
    brand_outcomes = sections.get("brand_outcomes") or []
    risk_radar = sections.get("risk_radar") or []
    activation = sections.get("activation_kit") or []
    operator_job_story = scope.get("operator_job_story") or "Not provided"
    approach_hints = ", ".join(scope.get("approach_hints", [])) or "Not provided"
    search_variants = ", ".join(scope.get("search_shaped_variants", [])) or "Not provided"
...
3. Keep copy short, factual, and grounded in the provided data. Write for an executive scanning on a phone, not for a schema browser.
   When describing `strengths` or `failure_modes`, use the voice of a trusted peer warning another operator—be candid about where each approach breaks.
...
@analysis_mcp.tool()
def generate_risk_radar(signals_json: str, sections_json: str) -> str:
    """Outline risk radar covering failure modes, timing traps, and detection lines."""

    signals = _loads(signals_json, [])
    sections = _loads(sections_json, {})
    logger.debug("generate_risk_radar (%d signals)", len(signals))
    prompt = _styled_prompt(
        f"""
...
@analysis_mcp.tool()
def generate_future_outlook(signals_json: str, sections_json: str, scope_json: str = "") -> str:
    """Create the future outlook ladder (6/12/24 month scenarios)."""

    signals = _loads(signals_json, [])
    sections = _loads(sections_json, {})
    scope = _loads(scope_json, {})
    logger.debug("generate_future_outlook (%d signals)", len(signals))
    prompt = _styled_prompt(
        f"""
...
@analysis_mcp.tool()
def generate_executive_letter(report_context_json: str) -> str:
    """Generate the executive overview letter for senior leaders."""

    context = _loads(report_context_json, {})
    prompt = _styled_prompt(
        f"""
...
@analysis_mcp.tool()
def generate_image_prompt_bundle(report_json: str) -> str:
    """Interpret the full report bundle and return 5 contextual image prompts."""

    # report_json is expected to be the entire bundle (markdown+sections)
    report = _loads(report_json, {})
    prompt = _styled_prompt(
        f"""
...
def write_executive_summary(
    ...
    """Write the executive summary and naming for the report."""
    ...
    quant = _loads(quant_json, {})
    scope = _loads(scope_json, {})
    logger.debug("write_executive_summary (%d sources, %d signals)", len(sources), len(signals))
    operator_job_story = scope.get("operator_job_story") or "Not provided"
    approach_hints = ", ".join(scope.get("approach_hints", [])) or "Not provided"
...
@analysis_mcp.tool()
def generate_image_briefs(
    sources_json: str, signals_json: str, sections_json: str, scope_json: str = ""
) -> str:
    """Create dark-mode friendly image briefs for hero, signal map, and case studies."""

    sources = _loads(sources_json, [])
    signals = _loads(signals_json, [])
    sections = _loads(sections_json, {})
    scope = _loads(scope_json, {})
    logger.debug("generate_image_briefs (%d signals)", len(signals))
    prompt = _styled_prompt(
        f"""

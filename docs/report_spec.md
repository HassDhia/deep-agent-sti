...
## Purpose
Signal Reports are the canonical operator intelligence artifact for STI. Every asset (social, slides, landing page) points back to the executive letter bundle (Markdown + PDF) generated by `run_report.py`—the legacy “Market-Path” name now simply aliases these files. The report bundle also exposes `public_markdown`, which is the essay-style Paul Graham narrative every downstream surface should render. The spec below mirrors the shared `REPORT_CONTRACT` in `config.py` so prompt writers, template authors, and QA all work from one contract.
...
6. Manual review the Markdown (`executive_letter.md`, a.k.a. the legacy `market_path_report.md`) and PDF to ensure typography and section ordering hold.

## Production Runbook
1. Set `OPENAI_API_KEY` (and `SEARXNG` env if needed).
2. `python3 run_report.py` (or invoke via automation pipeline).
3. Review `/sti_reports/<timestamp>/executive_letter.md` (the `market_path_report.md` alias also exists) and the matching PDF in a Markdown reader.
